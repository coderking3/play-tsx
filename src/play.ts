 ...
 * pnpm play -t ./tsconfig.dev.json -w file.ts
 *
 * Debug mode:
 * pnpm play --debug file.ts    → Show debug information
 * pnpm play -d -w -f other     → Debug + watch + file
 *
 * List files:
 * pnpm play --list             → List all available .ts files
 * pnpm play -l                 → Short form
 *
 ...
 * 2. Environment variable PLAY_TSX_TSCONFIG
 * 3. PlayOptions.tsconfig configuration
 * 4. tsconfig.json in current directory
 * 5. Not found, don't pass (tsx uses default config)
 */
